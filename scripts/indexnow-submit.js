...
const urls = [
  ...
  'https://www.behindmemory.com/ai-image-generation/flux-kontext-pro',
  'https://www.behindmemory.com/ai-image-generation/flux-kontext-max',
  'https://www.behindmemory.com/ai-image-generation/gemini-2.5-flash-image',
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200 || res.statusCode === 202) {
          ...
          resolve({ endpoint: endpoint.name, success: false, status: res.statusCode, response: responseData });
        }
      });
    });
...
async function submitAllPages() {
  ...
  const results = await Promise.all(
    endpoints.map(endpoint => submitToEndpoint(endpoint))
  );
