...
export default function ClaimAnonymousPage() {
  ...
  useEffect(() => {
    setMounted(true)
    // 打印初始状态，便于调试
    addDebugInfo(`页面加载, 当前状态: mounted=${true}, status=${status}, letterId=${letterId}, isProcessing=${isProcessing}`)
  ...
  const claimLetter = async () => {
    ...
    if (!session?.user?.id) {
      ...
      setError(language === 'en' 
        ? 'You must be logged in to claim a letter' 
        : '您必须登录才能认领信件')
      setIsProcessing(false)
      return
    }
    ...
    try {
      ...
      toast({
        ...
        description: language === 'en' 
          ? 'Letter has been saved to your account' 
          : '信件已保存到您的账户',
      })
      ...
      setError(language === 'en' 
        ? 'Failed to save letter to your account' 
        : '无法将信件保存到您的账户')
