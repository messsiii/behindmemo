...
export default function Pricing() {
  ...
  const allQuestions = content[language].faqSections.reduce<Question[]>(
    ...
    const checkUserStatus = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const userIsVIP =
            userData.isVIP &&
            (!userData.vipExpiresAt || new Date(userData.vipExpiresAt) > new Date())
          setIsVIP(userIsVIP)
        }
      } catch (error) {
  ...
  const handleSubscribe = async () => {
    ...
    if (!session?.user) {
      ...
      toast({
        ...
        description: language === 'en' ? 'Redirecting to login page...' : '正在跳转到登录页面...',
      })
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          language === 'en' ? 'Failed to check subscription status' : '检查订阅状态失败'
        )
      }
      ...
      <div className="relative z-10 flex-1">
        <div className="max-w-7xl mx-auto px-6 py-20 pt-32">
                ...
                <div className="space-y-6">
                    ...
                    {plan.originalPrice && (
                      <>
                        ...
                        <span className="text-[#cc8eb1] font-semibold">
                          {content[language].currentDiscount}{' '}
                          {Math.round((1 - Number(plan.price) / Number(plan.originalPrice)) * 100)}%{' '}
                  ...
                  <ul className="space-y-3">
                    {plan.features.map(feature => (
                      <li key={feature} className="flex items-start">
                        ...
                        <span
                          className={`text-gray-600 ml-2 ${language === 'en' ? 'font-serif' : 'font-serif-zh'}`}
                        >
                          {feature}
                        </span>
                      </li>
                  ...
                  <Button
                    ...
                    onClick={
                      ...
                      plan.name.toLowerCase().includes('订阅')
                        ? () => handleSubscribe()
                        : undefined
                    }
                    disabled={
              ...
              {creditPackages[language].map(pkg => (
                ...
                >
                  ...
                  <div className="p-5 flex flex-col h-full">
                    <div className="mb-3">
                      <h3
                        className={`text-xl font-semibold ${language === 'en' ? 'font-serif' : 'font-serif-zh'}`}
                      >
                    ...
                    <div className="flex items-baseline gap-2 mb-4">
                      <span
                        className={`text-3xl font-bold text-gray-900 ${language === 'en' ? 'font-serif' : 'font-serif-zh'}`}
                      >
                        ${pkg.price}
                      </span>
                      {pkg.originalPrice && (
                    ...
                    <div
                      className={`text-sm text-gray-700 mb-4 ${language === 'en' ? 'font-serif' : 'font-serif-zh'}`}
                    >
                      {pkg.bestValue
                        ? language === 'en'
          ...
          <div className="text-center mb-16">
            <p
              className={`text-sm text-gray-600 ${language === 'en' ? 'font-serif' : 'font-serif-zh'}`}
            >
              ...
              <Link
                href="/privacy"
                className="text-[#738fbd] hover:text-[#cc8eb1] transition-colors"
              >
